...
export class GLMAdapter implements ModelAdapter {
  id = 'glm5';
  name = 'GLM-5 (ZhipuAI)';
  private apiUrl = 'https://api.z.ai/api/coding/paas/v4/chat/completions';
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = https.request(
        ...
        (res) => {
          ...
          res.on('end', () => {
            try {
              const json = JSON.parse(data) as GLMResponse;
  ...
  private getApiKey(): string {
    ...
    for (const path of paths) {
      try {
        ...
        const data = JSON.parse(content) as AuthProfiles;
