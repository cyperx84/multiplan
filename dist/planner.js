...
import { getAdapter, getAvailableModels } from './models/index.js';
...
export async function run(config) {
    ...
    const planPromises = modelIds.map(async (modelId) => {
        ...
        try {
            ...
            if (config.verbose) {
                console.log(`[multiplan]   ✓ ${adapter.name} done (${durationMs}ms)`);
            }
        }
        catch (error) {
            const durationMs = Date.now() - startTime;
            ...
            results.push(result);
            if (config.verbose) {
                console.log(`[multiplan]   ✗ ${adapter.name} failed: ${result.error}`);
            }
        }
    });
